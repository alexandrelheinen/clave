...
All eight landed, one pull request each, the gate green on every one. Three
things changed on the way and are recorded where they happened rather than
here: `Cue` lost the `previous` field the design gave it, the `AC-TRACK-52`
fixture became a pinned submodule texture after a generated symbol proved
undecodable, and `AC-TRACK-48` was closed in two halves because the boundary
could not be the only path to a track until a track existed.
...
- [x] `belt_frame.py` with `Footprint`, `propagate`, `elapsed_seconds`.
- [x] Ban `time.monotonic_ns` inside the package and add the test that greps for
  it, so every instant stays an argument.
- [x] `NadirOptics.metres_per_pixel(height_meters, render_height)`, taking the
  object's upper surface rather than the belt plane, and taking the render size
  separately from `SensorSpec.resolution`.
- [x] Derive the window exit from `ReachReport.window_edges[1]` rather than
...
- [x] `evidence.py`: `Role`, the five payloads, the `Evidence` envelope, with
  `__post_init__` refusing a payload that carries a channel or a grasp.
- [x] `sensors.py`: read the `cameras` block into `SensorSpec`, look up by role,
  never by id.
- [x] `intake.py`: `Deployment`, and a gate that refuses `GroundTruth` under
  `HARDWARE` naming the source that offered it.
- [x] Put the gate on the only path into a track, so no caller reaches `Tracker`
  around it, and test that the bypass does not exist.
- [x] Remove the two `DETECTION_CAMERA = "gate_wide"` constants in
  `clave.data.recorder` and `clave.runtime.loop`, resolving by role instead.
- [x] Add the test that no camera id declared in `configs/world/sorting_line.yml`
...
- [x] `fusion.py`: the twelve-slot posterior with a configured floor.
- [x] `weight_of(confidence, elapsed_seconds)` accepting no `source_id`, with
  elapsed clamped at zero.
- [x] The fold table as an explicit dict with one documented miss branch.
- [x] The mass band over `clave.world.config.Range`, narrowing when a GTIN
  resolved a packaging mass.
- [x] The source-swap test: fold two disagreeing readings, fold them again with
  `source_id` and `role` exchanged, assert equality to 1e-12.
- [x] The out-of-order test: fold a reading older than `last_updated` and assert
...
- [x] `association.py`: `Cue` as a closed six-field dataclass, `Association`,
  the `Associator` protocol.
- [x] `SimulatorIdentity`, asymmetric by design: a `GroundTruth` cue matches by
...
- [x] `track.py`: `Track`, `WasteObject` with `provenance` and without
  `channel`, grasp geometry as computed properties, `settle`.
- [x] `tracker.py`: the one mutable driver.
- [x] `tests/tracker/association_contract.py`, a shared conformance suite not
  named `*_test.py`, so `learned-tracker` runs the identical clauses against its
  model.
- [x] Correct `docs/perception-contract.md` to drop `channel` from the record
...
- [x] `adapters/render.py` as the only MuJoCo importer, importing inside its
  functions.
- [x] `PixelMask` as run-length triples, so the adapter's input is a literal.
- [x] Second-moment extraction for the oriented footprint, with `oriented` set
  false when the eigenvalues are too close to separate.
- [x] Discard the geometry name, and add the test that no value in the resulting
  `Evidence`, `Track` or `WasteObject` is derivable from the producing object's
  `object_id`.
- [x] The resolution-invariance test: render one instant at 320 by 240 and 640
...
- [x] `codes.py`: symbology, the UPC-A and EAN-13 check digit computed here
  rather than trusted from the detector, the decoder protocol, the OpenCV
  implementation.
- [x] `configs/perception/packaging.yml`, GTIN to a packaging bill of materials,
  including `037600138727` which the potted meat can actually carries.
- [x] The code-derived prior, weighted by which component is plausibly the one
  being looked at, never overriding a confident visual reading.
- [x] An unknown GTIN yields a `Code` with no bill of materials.
- [x] Move `opencv-python-headless` into the `dev` extra so the decoder runs on
...
- [x] `measure_decode_yield` behind a `FrameSource` protocol so the counting is
  testable against a fake.
- [x] Report every denominator: decodes per object crossing the gate, and
  decodes per object inside a code camera's measured band.
- [x] Record the achieved figures in `docs/measurements.md`. The current
...
- [x] Add a fifth camera of an existing role in a test fixture, assert the
  record's key set is unchanged, and assert the new `source_id` reached a
  track's contributor set so the test is not vacuous.
- [x] Assert that removing the camera producing a configured role fails at load
