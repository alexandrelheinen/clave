...
def test_the_runtime_hands_every_published_decision_to_its_sink(
    ...
    with Bridge(
        ...
        assert bridge.submit(build(0.0, 0.0, 0.36, 0.9)).accepted
        assert bridge.submit(build(0.9, 0.0, 0.36, 0.9)).overridden
        assert bridge.submit(build(0.0, 0.0, 0.36, 0.1)).accepted
