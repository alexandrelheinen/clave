...
//!
//! Covers `AC-BUILD-01` through `AC-BUILD-05`. These criteria are properties
...
/// AC-BUILD-01: the validation script runs formatting, lint, test,
...
/// AC-BUILD-02: continuous integration runs that same script rather than a
...
/// AC-BUILD-03: the coverage gate fails below 80 percent of lines.
#[test]
fn the_coverage_gate_fails_below_eighty_percent_of_lines() {
    assert!(VALIDATE_SCRIPT.contains("--fail-under-lines 80"));
}

/// AC-BUILD-04: a release build keeps overflow checks on, so an arithmetic
...
/// AC-BUILD-04: the two crates on the decision path carry the hardened lint
...
/// AC-BUILD-05: the documentation build denies warnings, so a reference that
...
/// AC-BUILD-01: the toolchain is pinned with the components every gate needs,
