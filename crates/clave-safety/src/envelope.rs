...
///
/// The envelope is a sphere intersected with a box, which is not the shape of
/// a real workspace. It over-permits near the edges of reach, so it is an
/// upper bound on what is safe rather than a guarantee.
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct Envelope {
    arm_base: BeltPoint,
    reach_radius_meters: f64,
...
impl Envelope {
    ...
    pub fn from_json(bytes: &[u8]) -> Result<Self, SafetyError> {
        ...
        let base = triple(&value, "arm_base_meters")?;
        let envelope = Self {
            arm_base: BeltPoint::new(base[0], base[1], base[2]),
            reach_radius_meters: number(&value, "reach_radius_meters")?,
    ...
    /// Returns the point the arm is mounted at, in belt frame meters.
    #[must_use]
    pub const fn arm_base(&self) -> BeltPoint {
        self.arm_base
    }

    /// Returns how far the effector reaches from its base, in meters.
    #[must_use]
    pub const fn reach_radius_meters(&self) -> f64 {
        self.reach_radius_meters
    }
    ...
    fn within_reach(&self, point: BeltPoint) -> bool {
        let dx = point.x_meters() - self.arm_base.x_meters();
        let dy = point.y_meters() - self.arm_base.y_meters();
        let dz = point.z_meters() - self.arm_base.z_meters();
        dx * dx + dy * dy + dz * dz <= self.reach_radius_meters * self.reach_radius_meters
    }

    /// Refuses an envelope that describes no usable volume.
    fn validate(&self) -> Result<(), SafetyError> {
        if !(self.reach_radius_meters.is_finite() && self.reach_radius_meters > 0.0) {
            return Err(SafetyError::InvalidEnvelope {
                reason: "reach_radius_meters is not a positive finite number".to_owned(),
            });
        }
        for (key, (low, high)) in [
